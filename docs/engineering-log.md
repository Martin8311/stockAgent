...
- 阶段：8
- 现象：登录后虽然已经按角色拆分了业务工作台和后台管理，但第一屏仍然偏“导航壳”，用户需要继续向下找持仓、模型、Sandbox、Skill 等模块才能知道当前状态。
- 影响：真实用户进入系统时最先关心的是“我现在资产怎样、风险在哪里、能不能分析”；管理员关心的是“有没有待审批、Sandbox 是否异常、模型和 Skill 是否可用”。如果第一屏不回答这些问题，产品会像工程 demo，而不是可日常使用的工作台。
- 原因：前几个阶段的数据已经分散在各模块状态里，但没有做角色视角的聚合。直接新增后端 dashboard API 会扩大当前阶段范围，也会引入新的测试和权限面。
- 解决方案：先在前端基于已有接口数据聚合轻量概览：业务侧展示市值、未实现盈亏、持仓数量、画像完整度、最大持仓、风险关注、AI 可用性和最近分析；后台侧展示待审批、Sandbox 队列、模型可用性、生效 Skill、平台健康、行情数据源和 Skill 版本流。
- 验证方式：执行前端 `npm run build`，TypeScript 与 Vite 构建通过。
- 面试表达：我把 UI 打磨从“页面美化”推进到“角色任务建模”。概览层复用已有领域数据，不急着新增 dashboard API，既能提升真实用户体验，也控制了阶段复杂度和权限风险。

### 登录入口和角色工作区需要拆开，避免真实用户被演示型页面干扰

- 阶段：8
- 现象：前端最初把登录卡片、持仓、行情、AI 分析、Sandbox、Skill 治理和系统流水线堆在同一个页面里。它适合开发阶段快速验证接口，但不像真实产品，普通投资用户会看到大量治理和监控信息。
- 影响：业务用户容易被后台概念干扰；管理员也缺少集中处理模型目录、Sandbox、审批和运行状态的入口；简历演示时很难说明权限、信息架构和用户旅程。
- 原因：前几个阶段优先交付闭环功能，所有模块自然聚合到一个 MVP 页面。进入阶段 8 后，重点从“能跑”转向“像真实系统一样可用”，需要把业务体验和运维治理体验拆分。
- 解决方案：前端新增 `business/admin` 工作区状态；未登录时渲染独立登录页；登录后根据角色默认跳转，普通用户进入业务工作台，管理员进入后台管理；管理员可以在顶部切换回业务视图。业务区保留投资画像、持仓、行情和 AI 分析；后台区集中展示交付流水线、合规披露、模型目录、Sandbox 执行和 Skill 治理。
- 验证方式：执行前端 `npm run build`，TypeScript 与 Vite 构建通过。
- 面试表达：我没有只做视觉美化，而是按真实用户旅程重做信息架构。普通用户关注投资研究闭环，管理员关注治理、监控和审批；前端角色跳转提升体验，后端权限仍由 Spring Security 兜底。
...
- 阶段：6
- 现象：项目需要支持策略回测、压力测试和后续 Skill 验证，但直接执行用户或模型生成脚本会带来文件读取、环境变量泄露、网络访问、命令执行、无限循环和审计绕过风险。
- 影响：如果阶段 6 直接开放 Python、JavaScript 或 Shell，项目演示时看起来强大，但安全边界薄弱；一旦接入真实 API Key、用户持仓或付费模型，风险会迅速放大。
- 原因：Sandbox 的核心不是“能跑任意代码”，而是“可控执行”。当前项目还没有容器隔离、异步任务队列、审批工作台和资源配额系统，直接开放任意脚本会过早扩大攻击面。
- 定位过程：复盘阶段 5 multi-agent 后发现，最先需要的执行能力是回测和压力测试这两类可白名单化任务；它们可以用 DSL 描述，不需要系统命令或外部网络。
- 解决方案：新增 `SandboxTask` 任务表、`SandboxPolicyService` 安全准入、`SafeLocalSandboxExecutor` 白名单执行器和 `/api/sandbox/tasks` API。危险关键字直接 `REJECTED`，杠杆/真实交易/外部接口/付费数据等高风险意图进入 `PENDING_APPROVAL`，低风险任务同步执行并记录审计。
- 验证方式：后端测试覆盖安全 mock backtest、基于当前持仓的 portfolio stress test、危险脚本拒绝、高风险脚本待审批和未登录拦截；前端构建验证 Sandbox 工作台类型和展示。
- 面试表达：我把 Sandbox 设计成“策略准入 + 白名单执行 + 任务持久化 + 审计 + 审批状态”的组合，而不是单纯跑脚本。这样既能展示 Agent 工具执行能力，也能说明安全合规边界。
...
- 阶段：4
- 现象：AI 分析区域的模型选择下拉框只能停留在默认 Ollama，无法切换到 MiniMax 等模型。
- 影响：用户无法查看付费/测试模型的状态说明，模型选择能力看起来没有生效；也不利于展示“免费本地模型 + 付费模型预留”的产品能力。
- 原因：前端 `<option>` 使用了 `disabled={!model.enabled}`。当后端因为缺少 API Key、未开启付费权限或测试模式而返回 `enabled=false` 时，浏览器会禁止用户选择该模型。
- 定位过程：检查 `App.tsx` 的 `selectedModelId`、`aiModels` 和 `<select>` 绑定，确认状态本身可以更新，但 option 被禁用；同时 `loadAiModels` 只保留已启用模型，也会在刷新目录时把禁用模型重置回默认模型。
- 解决方案：下拉框允许选择所有目录模型，选项文本显示启用/未启用状态；运行按钮继续用 `selectedModel.enabled` 控制，防止提交不可用模型；下拉框下方展示当前模型 `statusNote`，说明为什么不可用或如何启用。
- 验证方式：执行前端 `npm run build`，TypeScript 与 Vite 构建通过。
- 面试表达：模型“可选择”和“可调用”是两个不同状态。前端不应该把不可调用模型从选择体验里隐藏掉，而应允许查看治理状态，再在执行入口做权限和配置校验。
